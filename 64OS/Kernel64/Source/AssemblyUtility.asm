...
kInPortWord:
    ...
    mov rdx, rdi    ; RDX 레지스터에 파라미터 1(포트 번호)를 저장
    mov rax, 0      ; RAX 레지스터를 초기화
    in ax, dx       ; DX 레지스터에 저장된 퐅트 어드레스에서 두 바이트를 읽어
...
kReadTSC:
    ...
    shl rdx, 32     ; RDX 레지스터에 있는 상위 32비트 TSC 값과 RAX 레지스테에 있는
...
%macro KSAVECONTEXT 0           ; 파라미터를 전달받지 않는 KSAVECONTEXT 매크로 정의
        ; RBP 레스트터부터 GS 세그먼트 셀렉터까지 모두 스택에 삽입
        push rbp
        push rax
        push rbx
        push rcx
        ...
        push rax    ; 삽입할 수 없으므로, RAX 게지스터에 저장한후 스택에 삽입
        mov ax, es
        push rax
        push fs
        push gs
...
kSwitchContext:
    ...
    mov qword[rid + (23 * 8) ], rax

    mov rax, rbp                        ; RBP에 저장된 RSP 레지스터 저장
    add rax, 16                         ; RSP 레지스터는 push rbp와 Retrun Address를 제외한 값으로 저장
    mov qword[rid + (22 * 8) ], rax

    pushfq                              ; RFLAGS 레지스터 저장
    pop rax
    mov qword[rid + (21 * 8) ], rax

    mov ax ,cs                              ; CS 레지스터 저장
    mov qword[rid + (20 * 8) ], rax

    mov rax, qword[rbp + 8]                 ; RIP 레지스터를 Retrun Address로 설정하여 다음 콘텍스트 복원시에
    mov qword[rid + (19 * 8) ], rax         ; 이함수를 호출한 다음 위치로 이동하게 함

    ; 저장한 레지스터를 복구한 후 인터럽트가 발생했을 때처럼 나머지 콘텍스트를 모두 저장
    pop rax
    pop rbp

    ; 가장 끝부분에 SS, RSP, FLAGS, CS, RIP 레지스터를 저장했으므로, 이전 영역에
...
kSetTS:
    ...
    mov rax, cr0        ; CR0 컨트롤러 레지스터의 값을 RAX 레지스터로 저장
    or rax, 0x08        ; TS 비트(비트 7)을 1로 설정
    mov cr0, rax        ; TS 비트가 1로 설정된 값을 CR0 컨트롤러 레지스터로 저장

    pop rax             ; 스택에서 RAX 레지스터의 값을 복원
    ret

; CR0 컨트롤 레지스터의 TS 비트를 0로 설정
;   PARAM: 없음
kClearTS:
...
kEnableGlobalLocalAPIC:
    push rax            ; RDMSR과 WRMSR에서 사용라는 레지스터를 모두 스택에 저장
    push rcx
    push rdx

    ; IA32_APIC_BASE MSR에 설정된 기본 값을 읽어서 전역 APIC 비트를 활성화 
