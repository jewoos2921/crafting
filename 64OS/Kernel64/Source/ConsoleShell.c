...
static void kShowHDDInformation(const char *pcParameterBuffer) {
    ...
    kPrintf("Model Numer:\t %s\n", vcBuffer);

    /// 시리얼 번호 출력
    kMemCpy(vcBuffer, stHDD.vwSerialNumber, sizeof(stHDD.vwSerialNumber));
    vcBuffer[sizeof(stHDD.vwSerialNumber) - 1] = '\0';
    kPrintf("Serial Numer:\t %s\n", vcBuffer);
...
static void kReadSector(const char *pcParameterBuffer) {
    ...
    /// 파라미터 리스트를 초기화하여 LBA 어드레스부터 섹터 수 추출
    kInitializeParameter(&stList, pcParameterBuffer);
...
static void kWriteSector(const char *pcParameterBuffer) {
    ...
    if (kWriteHDDSector(TRUE, TRUE, dwLBA,
                        iSectorCount, pcBuffer) == iSectorCount) {
        kPrintf("Write Fail\n");
        return;
    }
    kPrintf("LBA [%d], [%d] Sector Read Success~!!", dwLBA, iSectorCount);
...
static void kCreateFileInRootDirectory(const char *pcParameterBuffer) {
    ...
    if ((iLength > (FILE_SYSTEM_MAX_FILE_NAME_LENGTH - 1)) || (iLength == 0)) {
...
static void kShowRootDirectory(const char *pcParameterBuffer) {
    ...
    pbClusterBuffer = kAllocateMemory(FILE_SYSTEM_SECTOR_SPER_CLUSTER * 512);
    ...
    pstEntry = (DIRECTORY_ENTRY *) pbClusterBuffer;
    iTotalCount = 0;
    dwTotalByte = 0;
    ...
    for (i = 0; i < FILE_SYSTEM_MAX_DIRECTORY_ENTRY_COUNT; i++) {
        ...
        if ((iCount != 0) && ((iCount % 20) == 0)) {
            kPrintf("Press Any Key to Coninue... ('q' is exit) : ");
            if (kGetCh() == 'q') {
                kPrintf("\n");
                break;
            }
...
static void kTestFileIO(const char *pcParameterBuffer) {
    ...
    /// r옵션은 파일을 생성하지 않으므로 테스트 파일이 없으면 NIL이 되어야함
    ...
    /// w옵션은 파일을 생성하므로 정상적으로 핸들이 반환되어야 함
    ...
    /// 열린 핸들로 쓰기 수행
    for (i = 0; i < 100; i++) {
        kMemSet(pbBuffer, i, FILE_SYSTEM_CLUSTER_SIZE);
        if (fwrtie(pbBuffer, 1, FILE_SYSTEM_CLUSTER_SIZE, pstFile) != FILE_SYSTEM_CLUSTER_SIZE) {
            kPrintf("[Fail]\n");
    ...
    /// 파일의 처음으로 이동
    fseek(pstFile, -100 * FILE_SYSTEM_CLUSTER_SIZE, SEEK_END);

    /// 열린 핸들로 읽기 수행 후 데이터 검증
    for (i = 0; i < 100; i++) {
        /// 파일을 읽음
        if (fread(pbBuffer, 1, FILE_SYSTEM_CLUSTER_SIZE, pstFile) != FILE_SYSTEM_CLUSTER_SIZE) {
            kPrintf("[Fail]\n");
            return;
        }

        /// 데이터 검사
        for (j = 0; j < FILE_SYSTEM_CLUSTER_SIZE; j++) {
    ...
    /// 파일의 내용을 읽어서 버퍼로 복사
    fseek(pstFile, -100 * FILE_SYSTEM_CLUSTER_SIZE, SEEK_CUR);
    ...
    for (i = 0; i < 100; i++) {
        ...
        /// 데이터를 씀
        if (fwrtie(vbTempBuffer, 1, dwByteCount, pstFile) != dwByteCount) {
            kPrintf("[Fail]\n");
            break;
        } else {
            kPrintf("[Pass]\n");
    ...
    /// 맨마지막으로 이동하여 1바이트를 써서 파일의 크기를 4MB로 만듬
    fseek(pstFile, dwMaxFileSize - 1, SEEK_SET);
    fwrtie(&i, 1, 1, pstFile);
    ...
    for (i = 0; i < 100; i++) {
        dwByteCount = (kRandom() % (sizeof(vbTempBuffer) - 1)) + 1;
        dwRandomOffset = kRandom() % (dwMaxFileSize - dwByteCount);
        kPrintf("   [%d] Offset [%d] Byte [%d]...",
                i, dwRandomOffset, dwByteCount);
    ...
    for (i = 0; i < (2 * 1024 * 1024 / 1024); i++) {
        ...
        /// 1024바이트씩 파일을 씀
        if (fwrtie(pbBuffer + (i * 1024), 1, 1024, pstFile) != 1024) {
            kPrintf("[Fail]\n");
            return;
        } else {
            kPrintf("[Pass]\n");
    ...
    /// 열린 핸들로 일기 수행 후 데이터 검증. Random Write로 데이터가 잘못 저장될 수 있으므로 검증은 4MB 전체를 대사으로 함
    for (i = 0; i < (dwMaxFileSize / 1024); i++) {
        ...
        if (kMemCmp(pbBuffer + (8 * 1024), vbTempBuffer, 1024) != 0) {
            kPrintf("[Fail]\n");
            break;
        } else {
            kPrintf("[Pass]\n");
...
static void kTestPerformance(const char *pcParameterBuffer) {
    ...
    /// 클러스터 단위로 쓰는 테스트
    for (i = 0; i < (dwClusterTestFileSize / FILE_SYSTEM_CLUSTER_SIZE); i++) {
        if (fwrtie(pbBuffer, 1, FILE_SYSTEM_CLUSTER_SIZE, pstFile) !=
            FILE_SYSTEM_CLUSTER_SIZE) {
            kPrintf("Write Fail\n");
            /// 파일을 닫고 메모리를 해제함
            fclose(pstFile);
            kFreeMemory(pbBuffer);
    ...
    /// 클러스터 단위로 쓰는 테스트
    for (i = 0; i < (dwClusterTestFileSize / FILE_SYSTEM_CLUSTER_SIZE); i++) {
        if (fread(pbBuffer, 1, FILE_SYSTEM_CLUSTER_SIZE, pstFile) !=
            FILE_SYSTEM_CLUSTER_SIZE) {
            kPrintf("Read Fail\n");
            /// 파일을 닫고 메모리를 해제함
            fclose(pstFile);
    ...
    for (i = 0; i < dwOneByteTestFileSize; i++) {
        if (fwrtie(pbBuffer, 1, 1, pstFile) != 1) {
            kPrintf("Write Fail\n");
            /// 파일을 닫고 메모리를 해제함
            fclose(pstFile);
            kFreeMemory(pbBuffer);
...
static void kDownloadFile(const char *pcParameterBuffer) {
    ...
    DWORD dwTempSize;
    BYTE vbDataBuffer[SERIAL_FIFO_MAX_SIZE];
    ...
    if ((iFileNameLength > (FILE_SYSTEM_MAX_FILE_NAME_LENGTH - 1)) ||
        (iFileNameLength == 0)) {
        kPrintf("Too Long or Too Short File Name\n");
    ...
    // =====================================================================
    /// 데이터 길이가 수신될 때까지 기다린다는 메시지를 출력하고,
    /// 4바이트를 수신한 뒤 ACK를 전송
    // =====================================================================
    ...
    while (dwReceivedSize < dwDataLength) {
        /// 버퍼에 담아서 데이터를 씀
        dwTempSize = kReceiveSerialData(vbDataBuffer, SERIAL_FIFO_MAX_SIZE);
        dwReceivedSize += dwTempSize;

        /// 이번에 데이터가 수신된 것이 있다면 ACK 또는 파일 쓰기 수행
        if (dwTempSize != 0) {
            /// 수신하는 쪽은 데이터의 마지막까지 수신했거나 FIFO의 크기인 16바이트마다 한번씩 ACK 를 전송
            if (((dwReceivedSize % SERIAL_FIFO_MAX_SIZE) == 0) ||
            ...
            /// 쓰기 중에 문제가 생기면 바로 종료
            if (fwrtie(vbDataBuffer, 1, dwTempSize, fp) != dwTempSize) {
    ...
    // =====================================================================
    /// 전체 데이터의 크기와 실제로 수신받은 데이터의 크기를 비교하여 성공 여부를 출력란 뒤,
    /// 파일을 닫고 파일 시스템 캐시를 모두 비움
    // =====================================================================
    /// 수신된 길이를 비교해서 문제가 발생했느지를 표시
...
static void kStartApplicationProcessor(const char *pcParameterBuffer) {
    ...
    /// Bootstrap Processor의 APIC ID 출력
...
static void kStartSymmetricIOMode(const char *pcParameterBuffer) {
    ...
    if (pstMPManager->bUsePICMode == TRUE) {
        ...
        /// I/O I/O 포트 어드레스 0x23에 0x01을 전송하는 방법으로 IMCR 레지스터에 접근하여
        /// PIC 모드 비활성화
    ...
    /// 인터럽트를 불가로 설정
    kPrintf("Disaable CPU Interrupt Flag\n");
