...
void kDrawLine(int iX1, int iY1, int iX2, int iY2, COLOR stColor) {
    ...
    if (iDeltaX > iDeltaY) {
        ...
        /// X축 변화량이 Y축 변화량보다 작거나 같다면 Y축을 중심으로 직선을 그림
    else {
...
void kDrawCircle(int iX, int iY, int iRadius, COLOR stColor, BOOL bFill) {
    ...
    for (iCircleX = 1; iCircleX <= iCircleY; iCircleX++) {
        ...
        /// 중심원이 원의 외부에 있다면 아래에 있는 점 선택
        if (iDistance >= 0) {
            iCircleY--;

            /// 새로운 점에서 다시 원과의 거리 계산
            /// 시프트 연산으로 * 2를 대체
            iDistance += (-iCircleY << 1) - 1; // -2 * iCircleX -1;
        }
...
void kDrawText(int iX, int iY, COLOR stTextColor, COLOR stBackgroundColor,
    ...
    for (k = 0; k < iLength; k++) {
        ...
        /// 다음과 같이 계산 가능
        iBitmaskStartIndex = pcString[k] * FONT_ENGLISH_HEIGHT;

        /// 문자 출력
        for (j = 0; j < FONT_ENGLISH_HEIGHT; j++) {

            /// 이 라인에서 출력할 폰트 비트맵
            bBitmask = g_vucEnglishFont[iBitmaskStartIndex++];

            /// 현재 라인 출력
            for (i = 0; i < FONT_ENGLISH_WIDTH; i++) {
                /// 비트가 설정되어 있으면 화면에 문자색을 표시
                if (bBitmask & (0x01 << (FONT_ENGLISH_WIDTH - i - 1))) {
        ...
        /// 문자 하나를 다 출력했으면 폰트의 너비만큼 X좌표를 이동해서 다음 문자를 출력
        iCurrentX += FONT_ENGLISH_WIDTH;
    }
}