...
/// 콘솔의 정보를 관리하는 자료구조
CONSOLE_MANNAGER gs_stConsoleManager = {0,};
...
void kSetCursor(int iX, int iY) {
    ...
    /// CRTC 컨트롤 어드레스 레지스터(포트0x3D4)에 0x0E을 전송하여 상위 커서 위치 레지스터를 선택
    kOutPortByte(VGA_PORT_INDEX, VGA_INDEX_UPPERCURSOR);

    /// CRTC 컨트롤 레지스터(포트0x3D5)에 커서의 상위 바이트를 출력
    kOutPortByte(VGA_PORT_DATA, iLinearValue >> 8);


    /// CRTC 컨트롤 어드레스 레지스터(포트0x3D4)에 0x0F을 전송하여 하위 커서 위치 레지스터를 선택
    kOutPortByte(VGA_PORT_INDEX, VGA_INDEX_LOWERCURSOR);

    /// CRTC 컨트롤 레지스터(포트0x3D5)에 커서의 하위 바이트를 출력
...
void kGetCursor(int *piX, int *piY) {
    /// 저장된 위치를 콘솔 화면의 너비로 나눈 나머지로 X좌표를 구할 수 있으며, 화면 너비로 나누면 Y 좌표를 구할 수 있음
...
/// \n, \t 같은 문자가 포함된 문자열을 출력한 후 화면상의 다음 출력할 취치를 반환
int kConsolePrintString(const char *pcBuffer) {
    ...
    for (i = 0; i < iLength; ++i) {
        ...
        /// 출력할 위치가 화면의 최댓 값(80 * 25)을 벗어났으면 스크롤 처리
        if (iPrintOffset >= (CONSOLE_HEIGHT * CONSOLE_WIDTH)) {
            /// 가장 윗줄을 제외한 나머지를 한 줄 위로 복사
            kMemCpy((CHARACTER *) CONSOLE_VIDEO_MEMORY_ADDRESS,
                    (CHARACTER *) (CONSOLE_VIDEO_MEMORY_ADDRESS + CONSOLE_WIDTH * sizeof(CHARACTER)),
                    (CONSOLE_HEIGHT - 1) * CONSOLE_WIDTH * sizeof(CHARACTER));
//            kMemCpy(CONSOLE_VIDEO_MEMORY_ADDRESS,
...
BYTE kGetCh(void) {
    ...
    while (1) {
        ...
        while (kGetKeyFromKeyQeueue(&stData) == FALSE) { ;
        }

        /// 키가 눌렀다는 데이터가 수신되면 ASCII 코드를 반환
