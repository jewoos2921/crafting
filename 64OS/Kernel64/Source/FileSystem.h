...
/* 파일을 생성하는 알고리즘
 * 1) 루트 디렉터리에서 빈 디렉터리 엔트리를 검색
 * 2) 클러스터 링크 테이블에서 빈 클러스터(0x00)를 검색
 * 3) 빈 클러스터(0x00)의 링크 정보를 0xFFFFFFFF로 저장하여 할당된 상태이며 파일의 마지막 클러스터로 설정
 * 4) 빈 디렉터리 엔트리에 파일 정보인 파일명, 파일 크기, 시작 클러스터 인덱스를 갱신하여 엔트리를 등록
 * */

/* 파일을 삭제하는 알고리즘
 * 1) 루트 디렉터리에서 파일 이름이 일치하는 디렉터리 엔트리를 검색
 * 2) 첫 번째 클러스터 링크 정보에서 (0x00)로 저장해 빈 상태로 설정
 * 3) 빈 디렉터리 엔트리에 파일 정보인 파일명, 파일 크기, 시작 클러스터 인덱스를 초기화하여 엔트리를 제거
 * */
