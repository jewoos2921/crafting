...
/// 램 디스크를 관리하는 자료구조
static RDD_MANAGER gs_stRDDManager;
...
BOOL kInitializeRDD(DWORD dwTotalSectorCount) {
    ...
    if (gs_stRDDManager.pbBuffer == NIL) {
        return FALSE;
    }
...
BOOL kReadRDDInformation(BOOL bPrimary, BOOL bMaster,
    ...
    /// 자료구조 초기화
    kMemSet(pstHDDInformation, 0, sizeof(HDD_INFORMATION));

    /// 총섹터 수와 시리얼 번호, 모델 번호만 설정
...
int kWriteRDDSector(BOOL bPrimary, BOOL bMaster, DWORD dwLBA, int iSectorCount,
    ...
    /// LBA 어드레스부터 끝까지 쓸 수 있는 섹터 수와 써야 할 섹터 수를 비교해서 실제로 쓸 수 있는 수를 계산
