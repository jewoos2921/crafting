...
kReadCPUID:
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ; *pdwEAX
    mov esi, dword [ebp + 12]   ; 파라미터 1(dwEAX)를 EAX 레지스터에 저장
    mov dword [esi], eax        ; CPUID 명령어 실행

    ; *pdwEBX
    mov esi, dword[ebp + 16] ; 파라미터 2(pdwEAX)를 ESI 레지스터에 저장
    mov dword[esi], ebx         ; pdwEAX가 포인터이므로 포인터가 가리키는 어드레스에
                                ; EBX 레지스터의 값을 저장

    ; *pdwECX
    mov esi, dword[ebp + 20]    ; 파라미터 4(pdwECX)를 ESI 레지스터에 저장
    mov dword[esi], ecx         ; pdwEAX가 포인터이므로 포인터가 가리키는 어드레스에
                                ; ECX 레지스터의 값을 저장


    ; *pdwEDX
    mov esi, dword[ebp + 24]    ; 파라미터 5(pdwEDX)를 ESI 레지스터에 저장
    mov dword[esi], edx         ; pdwEDX 포인터이므로 포인터가 가리키는 어드레스에
                                ; EDX 레지스터의 값을 저장

    pop esi ; 함수에서 사용이 끝난 ESI 레지스터부터 EBP 레지스터 까지 스텍에
    pop edx ; 삽입된 값을 이용해 복원
    pop ecx ; 스택은 가장 마지막에 들어간 데이터가 가장 먼저 나오는
    pop ebx ; 자료 구조이므로 삽입의 역순으로
    pop eax ; 제거해야 함
    pop ebp     ; 베이스 포인터 레지스터 (EBP)복원
    ret         ; 함수를 호출한 다음 코드의 위치로 복귀


    ; IA-32e 모드로 전환하고 64 비트 커널로 수행
    ; PARAM: 없음
kSwitchAndExecute64bitKernel:
    ...
    or eax, 0x620       ; PAE 비트(비트 5)와 OSXMMEXCPT 비트(비트 10), OSFXSR 비트(비트 9)를 모두 1로 설정
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    mov ecx, 0xC0000080 ; IA32_EFER MSR 레지스터의 어드레스에 저장
    rdmsr               ; MSR 레지스터를 읽음

    or eax, 0x0100 ; EAX 레지스터에 저장된 IA32_EFER MSR의 하위 32 비트에서
                    ; LME 비트(비트 8)을 1로 설정
    ...
    ; 설정하여 캐시 기능과 페이징 기능을 활성화하고 TS비트(비트 3) = 1, EM비트(비트 2) = 0,
    ; MP 비트(비트 1) = 1로 설정하여 FPU를 활성화
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    mov eax, cr0            ; EAX 레지스터에 CR0 컨트롤 레지스터에 저장
    or eax, 0xE0000000      ; NW 비트 (비트29), CD 비트 (비트 30), PG 비트 (비트 31),
                            ; TS비트(비트 3), EM비트(비트 2), MP 비트(비트 1)을 모두 1로 설정

    xor eax, 0x60000000     ; NW 비트 (비트29), CD 비트 (비트 30), MP 비트(비트 2),
                            ; EM 비트(비트 2)을 XOR하여 0으로 설정
