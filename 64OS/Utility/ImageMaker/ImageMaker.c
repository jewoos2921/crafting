...
int main(int argc, char *argv[]) {
    ...
    // 파일 크기르 섹터 크기인 512바이트로 맞추기 위해 나머지 부분을 0x00으로 채움
    iBootLoaderSize = AdjustInSectorSize(iTargetFd, iSourceSize);
    printf("[INFO] %s size = [%d] and sector count = [%d]\n",
           argv[1], iSourceSize, iBootLoaderSize);

    // -----------------------------------------------------------------------------
    // 32 비트 커널을 열어서 모든 내용을 디스크 이미지 파일로 복사
    // -----------------------------------------------------------------------------
    printf("[INFO] Copy protected model kernel to image file\n");
    if ((iSourceSize = open(argv[2], O_RDONLY | O_BINARY)) == -1) {
        fprintf(stderr, "[ERROR] %s open fail\n", argv[2]);
        exit(-1);
    }
    ...
    // 부트 섹터의 5번째 바이트부터 ㅓ널에 대한 정보를 넣음
...
void WriteKernelInformation(int iTargetFd, int iTotalKernelSectorCount,
    ...
    if (lPosition == -1) {
        fprintf(stderr,
                "lseek fail. Return value=%ld, errno=%d, %d\n", lPosition, errno, SEEK_SET);
        exit(-1);
    }

    // 부트 로더를 제외한 총 섹터 수 및 보호 커널의 섹터 수 저장
